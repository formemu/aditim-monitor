...
class TaskCreateDialog(QDialog):
    ...
    def __init__(self, api_client, parent=None):
        ...
        self.setup_ui()
        self.load_data()
        self.load_dialog_styles()
    
    def load_dialog_styles(self):
        """Load dialog-specific styles"""
        try:
            style_path = os.path.join(os.path.dirname(__file__), "..", "resources", "styles", "dialogs.qss")
            if os.path.exists(style_path):
                with open(style_path, 'r', encoding='utf-8') as f:
                    self.setStyleSheet(f.read())
        except Exception as e:
            print(f"Error loading dialog styles: {e}")
